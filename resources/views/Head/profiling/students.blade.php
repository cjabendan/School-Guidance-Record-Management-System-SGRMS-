                ...
                <div class="table-container">
                    <div class="table-management">
                                ...
                                <div class="filters">
                                    <li>
                                        <a href="{{ url('Head/students') }}"
                                            class="tab {{ request('status') == null ? 'active' : '' }}">All</a>
                                        <a href="{{ url('Head/students') . '?status=seniorhigh' }}"
                                            class="tab {{ request('status') == 'seniorhigh' ? 'active' : '' }}">Senior
                                            Highschool</a>
                                        <a href="{{ url('Head/students') . '?status=juniorhigh' }}"
                                            class="tab {{ request('status') == 'juniorhigh' ? 'active' : '' }}">Junior
                                            Highschool</a>
                                        <a href="{{ url('Head/students') . '?status=elementary' }}"
                                            class="tab {{ request('status') == 'elementary' ? 'active' : '' }}">Elementary</a>
                                        <a href="{{ url('Head/students') . '?status=kindergarten' }}"
                                            class="tab {{ request('status') == 'kindergarten' ? 'active' : '' }}">Kindergarten</a>
                                    </li>
                                </div>
                        ...
                        <div class="search-bar">
                            ...
                            <form id="importForm" action="{{ route('Head.students.import') }}" method="POST"
                    ...
                    <div id="student-list">
                                ...
                                <div class="table-card">
                                    <div class="table-col title">
                                        <span class="status-circle {{ $statusClass }}"
                                            style="background: {{ $statusColor }} !important; vertical-align: middle; margin-right: 6px;"></span>
                                        {{ $row->s_id }}
                                    </div>
                                    <div class="table-col">{{ $name }}</div>
                                    ...
                                    <div class="table-col">
                                        <span
                                            style="display:inline-block;padding:4px 14px;border-radius:16px;font-weight:600;background:{{ $statusColor }}20;color:{{ $statusColor }};border:1px solid {{ $statusColor }};min-width:90px;text-align:center;">
                                            {{ $status }}
                                        </span>
                                    </div>
                                    <div class="table-col actions">
                                        ...
                                        <a href="javascript:void(0);" class="edit-btn" title="Edit"
                                            onclick="openAddEditModal('edit', { s_id: '{{ $row->s_id }}' })">
                                            <i class='bx bx-edit'></i>
                                        </a>
                                        <button type="button" class="archive-btn" title="Archive"
                                            onclick="openArchiveStudentModal('{{ $row->s_id }}')">
                        ...
                        @if ($students instanceof \Illuminate\Pagination\LengthAwarePaginator)
                            @component('components.student-pagination', ['paginator' => $students])
                            @endcomponent
                        @endif
                    </div>
                </div>
