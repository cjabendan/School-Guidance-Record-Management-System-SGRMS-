    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h5 class="modal-title" id="parentModalLabel">
          {{ isset($parent) ? 'Edit Parent' : 'Add Parent' }}
        </h5>
      </div>
      <form id="parentForm">
        <div class="modal-body">
          <div class="row">
            <div class="col-md-6 mb-3">
              ...
              <input type="text" class="form-control" id="firstName" name="first_name" required value="{{ $parent->first_name ?? '' }}">
            </div>
            <div class="col-md-6 mb-3">
              <label for="middleName" class="form-label">Middle Name</label>
              <input type="text" class="form-control" id="middleName" name="middle_name" value="{{ $parent->middle_name ?? '' }}">
            </div>
          </div>
          <div class="row">
            <div class="col-md-6 mb-3">
              <label for="lastName" class="form-label">Last Name</label>
              <input type="text" class="form-control" id="lastName" name="last_name" required value="{{ $parent->last_name ?? '' }}">
            </div>
            <div class="col-md-6 mb-3">
              <label for="sex" class="form-label">Sex</label>
              <select class="form-control" id="sex" name="sex" required>
                <option value="">Select Sex</option>
                <option value="Male" {{ (isset($parent) && $parent->sex == 'Male') ? 'selected' : '' }}>Male</option>
                <option value="Female" {{ (isset($parent) && $parent->sex == 'Female') ? 'selected' : '' }}>Female</option>
              </select>
            </div>
          </div>
          <div class="row">
            <div class="col-md-6 mb-3">
              ...
              <input type="text" class="form-control" id="contactNum" name="contact_num" required value="{{ $parent->contact_num ?? '' }}">
            </div>
            <div class="col-md-6 mb-3">
              <label for="email" class="form-label">Email</label>
              <input type="email" class="form-control" id="email" name="email" required value="{{ $parent->email ?? '' }}">
            </div>
          </div>
          <div class="row">
            <div class="col-md-6 mb-3">
        ...
        <div class="modal-footer">
          <button type="submit" class="btn btn-primary">
            {{ isset($parent) ? 'Update Parent' : 'Add Parent' }}
          </button>
        </div>
      </form>
    </div>
