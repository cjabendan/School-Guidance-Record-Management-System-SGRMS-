<div class="settings-form">
    ...
    <div class="recoveryCodes-container" style="display: flex; flex-direction: column; gap: 1.5rem; width: 70%;">
        @foreach (array_intersect_key($systemSettings, array_flip(['maintenance_mode', 'registration'])) as $key => $value)
            <div class="settings-flex-row" style="align-items: center; justify-content: space-between;">
                <div style="display: flex; flex-direction: column; gap: 1rem;">
                    <div>
                        ...
                        <p class="settings-form-subheading">
                            Manage the {{ str_replace('_', ' ', $key) }} functionality across the system.
                        </p>
                    </div>

                    <div style="display: flex; gap: 0.5rem; align-items: center;">
                        <button wire:click="toggle('{{ $key }}')"
                            class="settings-form-button {{ $value === 'on' ? 'Enabled' : 'Disabled' }}">
                            @if ($value === 'on')
                                <!-- Enabled -->
