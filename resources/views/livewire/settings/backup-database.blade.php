<div class="settings-form">
    ...
    <div style="display: flex; flex-direction: column; gap: 1.5rem; width: 58%;">
        ...
        <div>
            ...
            @if ($message && !in_array($backupStatus, ['running', 'completed', 'failed']))
        ...
        <div style="display: flex; flex-direction: column; gap: 1rem;">
            @if ($downloadPath && $backupStatus === 'completed')
                <button type="button" class="settings-form-button" style="background-color:#4caf50;"
                    wire:click="afterDownload('{{ $downloadPath }}')">
            ...
            @else
                <button wire:click="backup" wire:loading.attr="disabled" class="settings-form-button"
                    style="background-color: {{ $backupStatus === 'running' ? '#f0ad4e' : '' }};">
            ...
            @if ($message && in_array($backupStatus, ['failed', 'completed']))
                <p class="settings-form-subheading"
                    style="font-size:14px; color:{{ $backupStatus === 'failed' ? 'red' : 'green' }};">
                    {!! $message !!}
                </p>
            @endif
        </div>
